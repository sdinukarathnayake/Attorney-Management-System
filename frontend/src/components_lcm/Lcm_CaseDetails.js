...
function Lcm_CaseDetails() {
  ...
  const deleteCase = (id) => {
    axios
      ...
      .then(() => {
        ...
        navigate('/Lcm_Dashboard'); // Redirect to the AllCases page after deletion
      })
      .catch((err) => {
        alert(err.message);
      });
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      case "Ongoing":
        return { text: "Ongoing", color: "#ffcc00" }; // Yellow for ongoing
      case "Pending":
        return { text: "Pending", color: "#ff6600" }; // Orange for pending
      case "complete":
        return { text: "Complete", color: "#66cc00" }; // Green for complete
      default:
  ...
  return (
    <div>
      ...
      <div className="casedetails-show-container" ref={componentRef}>
        ...
        <div 
          className="cdetails-bookmark-container" 
      ...
      <div className="previous-button-container">
        <button
          type="button"
          className="casedetails-show-btn-edit"
          onClick={() => goToUpdatePage(caseItem._id)}
        >
          Update
        </button>
        <button
...
export default Lcm_CaseDetails;
